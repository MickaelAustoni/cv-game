...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private bool _playerGoToClickedPosition;
        ...
        private void CameraRotation()
        {
            ...
            // Cine machine will follow this target
            CinemachineCameraTarget.transform.rotation = Quaternion.Euler(_cinemachineTargetPitch + CameraAngleOverride, _cinemachineTargetYaw, 0.0f);
        }

        private void Move()
        {
            if (_input.moveClick) // On click to move
            {
                _playerGoToClickedPosition = true;

                // Rotate player direction
                var mouseRay = _mainCameraComponent.ScreenPointToRay(_input.mousePosition);
                var plane = new Plane(Vector3.up, transform.position);

                if (plane.Raycast(mouseRay, out var hitDist))
                {
            ...
            if (_input.move == Vector2.zero && !_playerGoToClickedPosition || Vector3.Distance(transform.position, _targetPosition) <= 0)
            {
                targetSpeed = 0.0f;
                _playerGoToClickedPosition = false;
            }
            ...
            // move the player
            if (_playerGoToClickedPosition) // With click
            {
